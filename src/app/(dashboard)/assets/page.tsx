    ...
    <div className="space-y-6 animate-in fade-in duration-500">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-1">Manage, track, and assign your enterprise devices.</p>
        </div>
      ...
      <Card className="bg-white border-slate-200 shadow-sm overflow-hidden">
        <CardContent className="p-0">
          {assets.length === 0 ? (
            <div className="p-10 text-center text-sm text-slate-500">No assets found. Add one or import from Excel.</div>
          ) : (
                ...
                <tbody className="divide-y divide-slate-100">
                  {assets.map((asset) => (
                    <tr key={asset.id} className="hover:bg-slate-50/80 transition-colors">
                      ...
                      <td className="px-6 py-4 text-slate-500 font-mono text-xs">{asset.assetTag}</td>
                      ...
                      <td className="px-6 py-4 flex justify-end">
                        <AssetActionMenu assetId={asset.id} assetName={asset.name} status={asset.status} />
                      </td>
                    </tr>
                  ))}
                </tbody>
